...
namespace TenchyShell.Workspaces.Tests;
...
public sealed class PlaceholderTests
{
    ...
    [Fact]
    public void HotkeyParserSupportsTabForWindowSwitcher()
    {
        var parsed = HotkeyParser.TryParse("Tab", out var combination, out var error);

        Assert.True(parsed, error);
        Assert.Equal(0x09u, combination.VirtualKey);
    }

    [Fact]
    public void WindowSwitcherStateCyclesForwardAndBackward()
    {
        var state = new WindowSwitcherState(new[]
        {
            new WindowSwitcherItem((IntPtr)101, "Notepad"),
            new WindowSwitcherItem((IntPtr)202, "WezTerm")
        });

        Assert.Equal((IntPtr)101, state.SelectedItem?.Handle);
        state.Move();
        Assert.Equal((IntPtr)202, state.SelectedItem?.Handle);
        state.Move();
        Assert.Equal((IntPtr)101, state.SelectedItem?.Handle);
    ...
    [Fact]
    public void WindowSwitcherStateScrollsWhenSelectionPassesVisibleRows()
    {
        var state = new WindowSwitcherState(Enumerable.Range(1, 10)
            .Select(index => new WindowSwitcherItem((IntPtr)index, $"Window {index}")));

        for (var index = 0; index < 5; index++)
        {
            state.Move(visibleItemCount: 3);
        }

        Assert.Equal(5, state.SelectedIndex);
        Assert.Equal(3, state.FirstVisibleIndex);
        Assert.InRange(state.SelectedIndex, state.FirstVisibleIndex, state.FirstVisibleIndex + 2);
    }

    [Fact]
    public void WindowSwitcherStateScrollsBackWhenSelectionMovesToPreviousRows()
    {
        var state = new WindowSwitcherState(Enumerable.Range(1, 10)
            .Select(index => new WindowSwitcherItem((IntPtr)index, $"Window {index}")));

        state.SelectLast(visibleItemCount: 3);
        state.Move(backwards: true, visibleItemCount: 3);

        Assert.Equal(8, state.SelectedIndex);
        Assert.Equal(7, state.FirstVisibleIndex);
        Assert.InRange(state.SelectedIndex, state.FirstVisibleIndex, state.FirstVisibleIndex + 2);
    }

    [Fact]
    public void WindowSwitcherStateSupportsPageAndBoundaryNavigation()
    {
        var state = new WindowSwitcherState(Enumerable.Range(1, 10)
            .Select(index => new WindowSwitcherItem((IntPtr)index, $"Window {index}")));

        state.MovePage(backwards: false, visibleItemCount: 3);
        Assert.Equal(3, state.SelectedIndex);
        state.SelectLast(visibleItemCount: 3);
        Assert.Equal(7, state.FirstVisibleIndex);
        state.SelectFirst(visibleItemCount: 3);
        Assert.Equal(0, state.FirstVisibleIndex);
        Assert.Equal(0, state.SelectedIndex);
    }

    [Fact]
    public void WindowSwitcherStateRemovesZeroHandlesAndDuplicates()
    {
        var state = new WindowSwitcherState(new[]
        {
            new WindowSwitcherItem(IntPtr.Zero, "Invalid"),
            new WindowSwitcherItem((IntPtr)101, "First"),
            new WindowSwitcherItem((IntPtr)101, "Duplicate")
        });

        Assert.Single(state.Items);
        Assert.Equal("First", state.SelectedItem?.Title);
    }
    ...
    [Fact]
    public void WindowServiceDoesNotCloseTenchyShellWindows()
    {
    ...
    private sealed class FakeWorkspaceWindowService : IWorkspaceWindowService
    {
        ...
        public string GetWindowTitle(IntPtr windowHandle) => $"Window {windowHandle}";
